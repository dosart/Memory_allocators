...
void fl_insert_first(fl_list_t *list, fl_mcb_t *node) {
  if (list == NULL)
    return;
  if (fl_is_empty(list))
    list->last = node;
...
void fl_remove(fl_list_t *list, fl_mcb_t *node) {
  if (list == NULL)
    return;
  if (list->head == NULL)
    return;
...
bool fl_is_empty(fl_list_t *list) {
  if (list == NULL)
    return true;
...
//   return (current_address + current->size + sizeof(fl_mcb_t) ==
//   next_address);
// }
